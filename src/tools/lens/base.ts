...
import { createErrorResponse } from "../common/response.js";
...
export abstract class BrowserToolBase implements ToolHandler {
  ...
  protected validatePageAvailable(context: ToolContext): ToolResponse | null {
    if (!this.ensurePage(context)) {
      return createErrorResponse("Lens Desktop page not initialized");
    }
    return null;
  }
  ...
  protected async safeExecute(
    ...
    try {
      ...
      if (!context.electronApp) {
        ...
        const { resetState } = await import("../../toolHandler.js");
        resetState();
      ...
      if (!context.page) {
        return createErrorResponse("Lens Desktop main page not initialized");
      }
